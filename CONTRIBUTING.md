...
3. Open a PR against `1.8` with:
   ...
   The [**CI workflow**](.github/workflows/ci.yml) must pass (unit tests on Ubuntu and Windows). For control-panel or autodetect changes, run the [**Integration Tests workflow**](.github/workflows/integration.yml) manually and link the Actions run. Link CI runs if you re-push after review feedback.
