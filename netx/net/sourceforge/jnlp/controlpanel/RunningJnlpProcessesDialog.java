...
public final class RunningJnlpProcessesDialog extends JDialog {
    ...
    private void refreshProcessList() {
        ...
        for (Iterator<RunningProcess> it = tracked.iterator(); it.hasNext();) {
            RunningProcess process = it.next();
            if (!containsPid(latest, process.getPid())) {
                it.remove();
            }
        }
        for (RunningProcess process : latest) {
            if (!containsPid(tracked, process.getPid())) {
                tracked.add(process);
            }
        }
    ...
    private JPanel buildProcessRow(final RunningProcess process) {
        ...
        String label = Translator.R("CacheRunningJnlpProcess", process.getPid(), process.getShortName());
