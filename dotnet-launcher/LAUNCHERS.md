...
Pattern mirrors `java.exe` / `javaw.exe`: GUI vs console entry points. `itweb-settings` is not a separate build — Maven and `publish-dotnet-launchers.ps1` copy `javaws` to `itweb-settings`.
...
| Linux | `packaging/icons/icedtea-web.png` | `share/pixmaps/` in dist; DEB/RPM install `/usr/share/pixmaps/` + `Icon=` in `.desktop` files |
