...
foreach ($file in $files) {
    ...
    if ($normalizedRelativeFile -ieq "bin\javaws.exe") {
        [void]$componentsXml.AppendLine("      <Icon Id=`"JavawsShortcutIcon.exe`" SourceFile=`"$source`" />")
        [void]$componentsXml.AppendLine("      <Shortcut Id=`"JavawsStartMenuShortcut`" Directory=`"ProgramMenuFolder`" Name=`"IcedTea-Web Java Web Start`" Target=`"[#$fileId]`" WorkingDirectory=`"INSTALLFOLDER`" Icon=`"JavawsShortcutIcon.exe`" IconIndex=`"0`" />")
    }
    if ($normalizedRelativeFile -ieq "bin\itweb-settings.exe") {
        [void]$componentsXml.AppendLine("      <Icon Id=`"SettingsShortcutIcon.exe`" SourceFile=`"$source`" />")
        [void]$componentsXml.AppendLine("      <Shortcut Id=`"SettingsStartMenuShortcut`" Directory=`"ProgramMenuFolder`" Name=`"IcedTea-Web Settings`" Target=`"[#$fileId]`" WorkingDirectory=`"INSTALLFOLDER`" Icon=`"SettingsShortcutIcon.exe`" IconIndex=`"0`" />")
    }
    if ($normalizedRelativeFile -ieq "bin\policyeditor.exe") {
        [void]$componentsXml.AppendLine("      <Icon Id=`"PolicyEditorShortcutIcon.exe`" SourceFile=`"$source`" />")
        [void]$componentsXml.AppendLine("      <Shortcut Id=`"PolicyEditorStartMenuShortcut`" Directory=`"ProgramMenuFolder`" Name=`"IcedTea-Web Policy Editor`" Target=`"[#$fileId]`" WorkingDirectory=`"INSTALLFOLDER`" Icon=`"PolicyEditorShortcutIcon.exe`" IconIndex=`"0`" />")
    }
